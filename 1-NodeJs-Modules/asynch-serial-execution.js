//Reading file data "Asychronously" using callbacks in serail order
...
function callbackFunction1(err, f1data) {
  ...
  fs.readFile("f2.txt", callbackFunction2); // Reads the file 2 data after file 1
}
...
function callbackFunction2(err, f2data) {
  ...
  fs.readFile("f3.txt", callbackFunction3); // Reads the file 3 data after file 2
}

function callbackFunction3(err, f2data) {
  if (err) {
    console.log("Error -> " + err);
  }
  console.log("File 3 content -> " + f2data);
}
...
// File 3 content -> I am File 3 content
